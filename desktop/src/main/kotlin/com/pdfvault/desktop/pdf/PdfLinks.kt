...
/** Where a clicked link goes: an external URL, or another page within the document. */
sealed interface LinkTarget {
    data class Web(val uri: String) : LinkTarget
    data class Page(val index: Int) : LinkTarget
}

/** A clickable link region on a page, positioned in normalized page coordinates (0..1, top-left). */
data class PdfLink(
...
/**
 * Extracts clickable link annotations for every page. Rectangles are normalized to the crop box,
 * top-left origin (same space as [HighlightRect]), so the reader can overlay them directly. Page
 * rotation isn't corrected. Returns a page-index → links map; pages without links are omitted.
 */
