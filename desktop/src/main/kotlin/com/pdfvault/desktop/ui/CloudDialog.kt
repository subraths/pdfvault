    ...
    fun run(register: Boolean) {
        scope.launch {
            ...
            try {
                ...
                // Close on success: with an imported account the main UI appears; without one the
                // S3 setup screen is underneath, ready for the one-time key + bucket step.
                onDismiss()
        ...
        text = {
            Column(Modifier.width(360.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                if (authState.signedIn) {
                    ...
                    Text(
                        "Sign in to restore your S3 account and recents, or create an account — " +
                            "you'll connect your S3 storage once and never enter it again.",
                        style = MaterialTheme.typography.bodySmall,
                        color = MaterialTheme.colorScheme.onSurfaceVariant,
                    )
